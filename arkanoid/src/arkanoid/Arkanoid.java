...
public class Arkanoid extends JFrame implements KeyListener {
	...
	/* CONSTANTS */
	public static final int PUNTUACION_FINAL = 168;
	...
	class ScoreBoard {
		...
		void increaseScore(Brick brick) {
			if(brick.color==Color.RED){
				score++;	
			}else if(brick.color==Color.GREEN) {
				score+=2;
	...
	class Ball extends GameObject {
		...
		void update(ScoreBoard scoreBoard, Paddle paddle) {
			...
			if(bricks.size()<=20) {
				x += velocityX*2*FT_STEP;
				y += velocityY *2* FT_STEP;
			}else if(bricks.size()<=10) {
				x += velocityX*3* FT_STEP;
				y += velocityY *3* FT_STEP;
			}else  {
				x += velocityX* FT_STEP;
				y += velocityY * FT_STEP;
			}
		...
		for (int iX = 0; iX < COUNT_BLOCKS_X; ++iX) {
			for (int iY = 0; iY < COUNT_BLOCKS_Y; ++iY) {
				...
				bricks.add(new Brick((iX + 1) * (BLOCK_WIDTH + 3) + 22,
						(iY + 2) * (BLOCK_HEIGHT + 3) + 20, Color.blue));
			}
				else if((iX%2==1) && (iY%2==1)) {
					bricks.add(new Brick((iX + 1) * (BLOCK_WIDTH + 3) + 22,
							(iY + 2) * (BLOCK_HEIGHT + 3) + 20, Color.red));
				}
				else if((iX%2==0) && (iY%2==0)) {
					bricks.add(new Brick((iX + 1) * (BLOCK_WIDTH + 3) + 22,
							(iY + 2) * (BLOCK_HEIGHT + 3) + 20, Color.green));
				}
				else{
					bricks.add(new Brick((iX + 1) * (BLOCK_WIDTH + 3) + 22,
							(iY + 2) * (BLOCK_HEIGHT + 3) + 20, Color.yellow));
				}
